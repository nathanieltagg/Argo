...
GalleryRecordComposer::GalleryRecordComposer(JsonObject& output, std::string filename, Long64_t jentry, const std::string options)
  : fOutput(output), fEntry(jentry), fOptions(options)
{
  ...
  {
    TimeReporter tt("-open-file");
    fEvent = std::unique_ptr<gallery::Event>(new gallery::Event({filename}));
    tt.addto(fStats);
  }
}
