...
std::string MakePng::writeToUniqueFile(const std::string& path)
{
  ...
  if(fd<0) {
    std::cerr << "MakePng::Couldn't open unique temporary file: " << strerror(errno) << " when writing to path " << path << std::endl;
    return "error";
  }
  ::write(fd,outdata,outdatalen);
  // Make world-readable
  fchmod(fd, 0000644);
  close(fd);
