...
int dump_sqlite(sqlite3* db, const std::string& select) {
    sqlite3_stmt *stmt = NULL;
    int rc = sqlite3_prepare_v2(db, select.c_str(), -1, &stmt, NULL);
    if (rc != SQLITE_OK)
        return rc;
    ...
    while (rc != SQLITE_DONE && rc != SQLITE_OK)
    {
        ...
        for (int colIndex = 0; colIndex < colCount; colIndex++)
        {
            ...
            {
                std::string valChar = (const char*) sqlite3_column_text(stmt, colIndex);
                printf("columnName = %s,Text val = %s", columnName, valChar.c_str());
            }
                     ...
                     ostream& errors)
{
  ...
  std::cout << "Read config: " << timer.Count() <<std::endl;
  ...
  for (auto const& pr : collection) {
    ...
    std::string label;    
    if(ps.get_if_present<string>("service_provider", label) ||  ps.get_if_present<string>("service_type", label)) 
    {
      if(label=="Geometry") {
        std::cout << ps.get<std::string>("GDML","unknown") << std::endl;
        // break;
      }
    };
    std::string moduletype;
    if(ps.get_if_present<string>("module_type", moduletype))
      if(moduletype == "RawDigitFilterUBooNE") 
      {
        std::cout << ps.get<int>("NumTicksToDropFront",0) << std::endl;
      }
  ...
  std::cout << "Finished reading parameters sets after " << timer.Count() << " s" << std::endl;
